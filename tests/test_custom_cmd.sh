...
"$PROG" run --once -- sh -c 'exit 7' > /dev/null 2>&1
...
"$PROG" run --max-retries 2 --backoff-base 1 --backoff-cap 1 \
...
unit="$("$PROG" print-unit --once -- ssh -W host:22 target)"
exec_line="$(printf '%s\n' "$unit" | grep '^ExecStart=')"
...
unit="$("$PROG" print-unit -- ssh -W host:22 target)"
